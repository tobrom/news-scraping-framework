...
#news <- url %>% 
#  html_nodes("article.di_teaser h1.di_teaser__heading") %>%
#  html_text() %>%
#  str_trim() %>%
#  str_replace("\n", "") %>%
#  str_replace_all("[\"]", "") 
...
#folders <- c("nyheter", "pressreleaser", "opinion", "bors")
#folders <- "nyheter"
...
ggplot(head(wordFrame, 20), aes(x = reorder(keyword, -count), y = count, fill = keyword)) + 
